...
public class Assorted {
    ...
    public static int toBinary(List<Integer> list) {
        int dec = 0;
        for (int i = 0; i<list.size(); i++){
            dec = dec + (int) (Math.pow(2,list.size()-i-1) * list.get(i));
    ...
    public static List<Integer> uniqueNumber(int lowerBound, int upperBound) {
        ...
        for (int i = lowerBound; i <= upperBound; i++){
            ...
            for (int j = 0; j < value.length(); j++){
                total += (int) Math.pow(Integer.parseInt(value.charAt(j) + ""), j+1);
            }
            if (total==i){
                ans.add(i);
            }
    ...
    public static List<Integer> filterNTimes(List<Integer> list, int n) {
        ...
        ArrayList<Integer> uniqueValues = new ArrayList<>(new HashSet<>(list));
        List<Integer> ans = new ArrayList<>();
        for (int i = 0; i < uniqueValues.size(); i++) {
            mapping.put(uniqueValues.get(i), 0);
        }

        for(int i = 0; i < list.size(); i++){
            if (mapping.get(list.get(i)) == n ) {
                continue;
            }
            mapping.put(list.get(i), mapping.remove(list.get(i)) + 1);
            ans.add(list.get(i));
        }
    ...
    public static int queueTime(List<Integer> queue, int tillsOpen) {
        ...
        while (queue.size() > 0 || counters.size() != 0) {
            while (counters.size() != tillsOpen && queue.size() != 0) {
                counters.add(queue.remove(0));
            }

            counters.replaceAll(integer -> integer - 1);

            counters.removeIf(j -> (j == 0));
            t++;
        }
        return t;
    }
